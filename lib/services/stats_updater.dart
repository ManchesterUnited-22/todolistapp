...
class StatsUpdater {
  ...
  Future<void> _onAuthChanged(User? user) async {
    ...
    try {
      final q = await FirebaseFirestore.instance.collection('report').where('uid', isEqualTo: uid).limit(1).get();
      if (q.docs.isEmpty) {
        // ignore: avoid_print
        print('StatsUpdater: no reports found for uid=$uid — creating sample report');
    ...
    _taskSub = FirebaseFirestore.instance
        ...
        .listen((snap) async {
      try {
        final tasks = snap.docs.map((d) => TaskViewModel.fromMap(d.data())).toList();
        ...
        try {
          ...
          final periodStart = Timestamp.fromDate(now.subtract(const Duration(days: 7)));
          ...
          final completedByPriority = <String, int>{'high': 0, 'medium': 0, 'low': 0};
          ...
          for (final task in tasks) {
            ...
            else if (priorityRaw.contains('thấp') || priorityRaw.contains('low') || priorityRaw.contains('thap')) pKey = 'low';
            priorityCounts[pKey] = (priorityCounts[pKey] ?? 0) + 1;

            final categoryRaw = task.category.trim().isEmpty ? 'Khác' : task.category.trim();
            categoryStats.putIfAbsent(categoryRaw, () => ReportCategoryStat(name: categoryRaw, count: 0, overdueCount: 0, lateCount: 0, overdueMinutes: 0));
            ...
            final isCompleted = task.stat.toLowerCase().contains('hoàn') || task.stat.toLowerCase().contains('hoan') || completed != null;
            if (isCompleted) {
              // Completed by priority
              completedByPriority[pKey] = (completedByPriority[pKey] ?? 0) + 1;
            }
            ...
            if (isCompleted && completed != null && due != null) {
              ...
              if (diff <= 0) {
                ...
                categoryStats[categoryRaw] = ReportCategoryStat(name: cs.name, count: cs.count, overdueCount: cs.overdueCount, lateCount: cs.lateCount + 1, overdueMinutes: cs.overdueMinutes + diff);
              ...
              final cs = categoryStats[categoryRaw]!;
              categoryStats[categoryRaw] = ReportCategoryStat(name: cs.name, count: cs.count, overdueCount: cs.overdueCount + 1, lateCount: cs.lateCount, overdueMinutes: cs.overdueMinutes + overdueMins);
          ...
          final report = ReportViewModel(
            ...
            topCategory: stats.categoryShares.isNotEmpty ? stats.categoryShares.first.name : '',
